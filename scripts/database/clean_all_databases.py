#!/usr/bin/env python3
"""
Complete database cleanup script for both Supabase (PostgreSQL) and Neo4j.
This will remove all data but preserve schema and constraints.
"""
...
class DatabaseCleaner:
    ...
    def clean_supabase(self):
        ...
        try:
            ...
            result = supabase.table("messages").delete().gte("id", 0).execute()
            logger.info(f"Deleted {len(result.data) if result.data else 'all'} messages")
            
            # Clean sessions table
            logger.info("Cleaning sessions table...")
            result = supabase.table("sessions").delete().gte("id", 0).execute()
            logger.info(f"Deleted {len(result.data) if result.data else 'all'} sessions")
            
            # Clean chunks table (foreign key references documents)
            logger.info("Cleaning chunks table...")
            result = supabase.table("chunks").delete().gte("id", 0).execute()
            logger.info(f"Deleted {len(result.data) if result.data else 'all'} chunks")
            
            # Clean documents table
            logger.info("Cleaning documents table...")
            result = supabase.table("documents").delete().gte("id", 0).execute()
            logger.info(f"Deleted {len(result.data) if result.data else 'all'} documents")
...
def main():
    ...
    print("\n⚠️  WARNING: This will DELETE ALL DATA from both Supabase and Neo4j databases!")
